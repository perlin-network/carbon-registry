...
import './addNewProgrammeUndp.scss';
...
const ProgrammeCreationComponent = () => {
  const { t } = useTranslation(['addProgramme', 'common']);
  ...
  const getCountryList = async () => {
    ...
    try {
      ...
      console.error('Error in getting country list', error);
    } finally {
  ...
  const getRegionList = async () => {
    ...
    try {
      ...
      console.error('Error in getting regions list', error);
    } finally {
      setLoadingList(false);
    }
  };
  ...
  const getOrganisationsDetails = async () => {
    ...
    try {
      ...
      console.error('Error in getting organisation list', error);
    } finally {
      setLoadingList(false);
    }
  };
  ...
  const onFinishStepOne = async (values: any) => {
    ...
    const proponentOrganisations =
      userInfoState?.companyRole !== CompanyRole.GOVERNMENT &&
      userInfoState?.companyRole !== CompanyRole.MINISTRY
        ? ownershipPercentage?.slice(1).map((item: any) => item.organisation)
        : ownershipPercentage?.map((item: any) => item.organisation);
    // const proponentTxIds = organisationsList
    //   .filter((x) => proponentOrganisations.includes(x.companyId))
    //   .map((x) => x.taxId);
    let logoBase64 = '';
    let logoUrls: any[] = [];
    ...
    console.log('proponentTxIds', proponentOrganisations);
    const propTaxIds =
      userInfoState?.companyRole !== CompanyRole.GOVERNMENT &&
      userInfoState?.companyRole !== CompanyRole.MINISTRY
        ? [userOrgTaxId, ...proponentOrganisations]
        : proponentOrganisations;
    console.log('propTaxIds', propTaxIds, userInfoState);
    ...
    if (totalPercentage !== 100) {
      ...
      console.info('stepOneData', programmeDetails);
      setLoading(false);
...
export default ProgrammeCreationComponent;
